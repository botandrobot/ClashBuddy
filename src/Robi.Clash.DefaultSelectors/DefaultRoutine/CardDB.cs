...
namespace Robi.Clash.DefaultSelectors
{
    ...
    public class CardDB
    {
        ...
        public void cardsAdjustment(Robi.Clash.Engine.NativeObjects.Logic.Client.UI.SpellBattleButton spellBtn)
        {
            var spell = spellBtn.SpellDeckSpell.Spell;
            if (spell == null || !spell.IsValid) return;
            cardName cName = cardNamestringToEnum(spellBtn.SpellDeckSpell.Spell.Name.Value, "19");
            ...
            if (cardsAdjustmentDB.ContainsKey(cName))
            {
                ...
                {
                    Card c = collectNewCards(spellBtn, false);
                    list.Add(c);
